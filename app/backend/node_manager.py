...
class BackendNode(Ros2NodeManager):
    ...
    def __init__(self, tinynav_db_path: str = '/tinynav/tinynav_db'):
        ...
        self.create_subscription(Path, '/planning/bezier_path', self._on_bezier_path, 1)
    ...
    def _on_bezier_path(self, msg: Path):
        pts = [
            {'x': p.pose.position.x, 'y': p.pose.position.y}
            for p in msg.poses
        ]
        with self._lock:
            self._bezier_path = pts
    ...
    def get_planning_snapshot(self) -> dict:
        with self._lock:
            ...
            snapshot = {
                ...
                'bezier_path': list(self._bezier_path),
